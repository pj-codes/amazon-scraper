...
class AmazonAPI:
    ...
    def get_price(self):
        ...
        try:
            ...
            if self.currency not in price:
                price = self.currency + price.strip()
            price = self.convert_price(price.strip())
        except NoSuchElementException:
            try:
                availability = self.driver.find_element_by_id("availability").text
                if "Available" in availability:
                    price = self.driver.find_element_by_id("unqualifiedBuyBox").text
                ...
                try:
                    price = self.driver.find_element_by_id("a-autoid-5-announce").text
                    price = self.convert_price(price)
                except Exception as e:
                    print(e)
    ...
    def convert_price(self, price):
        price = price.split(self.currency)[1].split("\n")[0]
        try:
            price = price.split("\n")[0] + "." + price.split("\n")[1]
        except:
            Exception()
